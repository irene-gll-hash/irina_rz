...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    low_variance_numeric = False
    for col in summary.columns:
        if col.is_numeric and col.std is not None:
            if col.std < 1e-6:
                low_variance_numeric = True
                break
    flags["has_low_variance_numeric"] = low_variance_numeric
    
    high_cardinality_categorical = False
    for col in summary.columns:
        if not col.is_numeric:
            unique_share = col.unique / summary.n_rows if summary.n_rows > 0 else 0
            if unique_share > 0.9:
                high_cardinality_categorical = True
                break
    flags["has_high_cardinality_categorical"] = high_cardinality_categorical
